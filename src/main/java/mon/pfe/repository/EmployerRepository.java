...
public interface EmployerRepository extends JpaRepository<Employer, Long> {
 ...
 public List<Employer> find_by_nom(@Param("x")String nom);
	...
	@Query("select emp from Employer emp  where emp.nom_utilisateur like :x")
	 ...
	 @Query("select emp from Employer emp where emp.nom_utilisateur = :username")
	    public Employer find_by_nom_utilisateur1(@Param("username") String nom_utilisateur); // Nouvelle méthode pour trouver un seul employé par son nom d'utilisateur
